 ...
 */
public class SnarkWrapperCustom implements Prover {
  ...
  public static void setLocal(boolean local) {
    SnarkWrapperCustom.local.set(local);
  }
  ...
  private SnarkWrapperCustom() {

  }

  private static final SnarkWrapperCustom INSTANCE;

  static {
    INSTANCE = new SnarkWrapperCustom();
  }

  public static SnarkWrapperCustom getInstance() {
  ...
  public Optional<Pair<Justification, String>> proveAndGetProof(Set<Formula> assumptions, Formula formula) {
    ...
    if (local.get()) {
      ...
      synchronized (interpreter) {
        ...
        capturer.start();
        LispObject result = interpreter.eval("(prove-from-axioms-yes-no " + assumptionsListString + goalString + " :verbose nil)");;
        proofString = capturer.stop();

        resultString = result.toString();
      }
    } else {
      proofString = "Distributed version not implemented";
      String url = null;
      try {
        url = "http://localhost:8000/prove?assumptions=" + URLEncoder.encode(assumptionsListString, "UTF-8") + "&goal=" + URLEncoder.encode(goalString, "UTF-8");
      } catch (UnsupportedEncodingException e) {
    ...
    if (resultString.equals("YES")) {
      return Optional.of(Pair.of(new TrivialJustification(assumptions, formula, ":FOLFromSnark"), proofString));
    } else {
      return Optional.empty();
    }
  ...
  public Optional<Pair<Value, String>> proveAndGetBindingWithProof(Set<Formula> assumptions, Formula formula, Variable variable) {
    ...
    if (resultString.isEmpty()) {
      ...
      try {
        return Optional.of(Pair.of(Reader.readLogicValueFromString(resultString), proofString));
      } catch (Reader.ParsingException e) {
        e.printStackTrace();
        return Optional.empty();
      }
